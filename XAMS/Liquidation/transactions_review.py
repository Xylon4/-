                ...
                # 所有操作为"输入后选择"的元素
                    ...
                    if value[n] == '置空':
                        ...
                        if menu[n] in ['资产类型', '高级查询_资产类型']:
                            ...
                            self.findxpath_click(f'//span[contains(text(),"{value[n]}")]')
                # 所有操作为"点击后选择"的元素
                elif menu[n] in ['市场类型',
                                 '高级查询_市场类型',
                                 '高级查询_清算类型'
                    ...
                    if value[n] not in a:
                        ...
                        findelement.click()
                        selectall = self.findxpath('/html/body/div[last()]//div[text()=" 全选"]')
                        action = ActionChains(self.driver)
                        action.double_click(selectall).perform()
                        findelement.click()
                    elif value[n] == '全选':
                        findelement.click()
                        self.findxpath_click(f'/html/body/div[last()]//div[text()=" {value[n]}"]')
                        findelement.click()
                    else:
                        findelement.click()
                        self.findxpath_click(f'/html/body/div[last()]//li[text()=" {value[n]}"]')
                        findelement.click()
                elif menu[n] in ['高级查询_清算类型']:
                    a = self.base.enumeration_list2(basedata[0], basedata[1], menu[n])
                    a.append('置空')
                    if value[n] not in a:
                        print(f'值"{value[n]}"输入错误，请检查')
                        ...
                        findelement.click()
                        selectall = self.findxpath('//li[text()=" 托管直连清算"]/../../preceding-sibling::div')
                        action = ActionChains(self.driver)
                        action.double_click(selectall).perform()
                        findelement.click()
                    elif value[n] == '全选':
                        findelement.click()
                        self.findxpath_click('//li[text()=" 托管直连清算"]/../../preceding-sibling::div')
                        findelement.click()
                    else:
                        findelement.click()
                        self.findxpath_click(f'//li[text()=" {value[n]}"]')
