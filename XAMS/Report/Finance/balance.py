...
from XAMS.Report.conftest import sheet48, Excel_basedata_zs
from XAMS.Tool.test_excel import TestExcel
from XAMS.basepage_XAMS import BasePageXams
import math


class Balance(BasePageXams):
    # 模拟操作自动化案例-浙商
    def balance_excel(self, menu, value):
        print(menu)
        print(value)
        self.base = TestExcel()
        basedata = [Excel_basedata_zs, sheet48]
        # 点击一级菜单
                ...
                # 所有操作为"点击后选择"的元素
                    ...
                    if value[n] not in a:
                        ...
                        if menu[n] in ['是否穿透']:
                            ...
                            if value[n] == '置空':
                                selectall = self.findxpath('//div[text()=" 全选"]')
                                action = ActionChains(self.driver)
                                action.double_click(selectall).perform()
                                findelement.click()
                            elif value[n] == '全选':
                                self.findxpath_click('//div[text()=" 全选"]')
                                findelement.click()
                            else:
                                self.findxpath_click(f'//li[text()=" {value[n]}"]')
            n = n + 1
    ...
    # 升级对比自动化案例-浙商
    def balance_compare(self, menu, value):
        print(menu)
        print(value)
        self.base = TestExcel()
        basedata = [Excel_basedata_zs, sheet48]
