...
from XAMS.Report.conftest import sheet7, Excel_basedata_zs
...
class ProductQuantity(BasePageXams):
    ...
    def product_quantity_excel(self, menu, value):
        ...
        n = 2
        while n < l:
            targetsheet = self.base.sheet_xpath_dic(Excel_basedata_zs, sheet7)
    ...
    def product_quantity_compare(self, menu, value):
        ...
        while n < l:
            ...
            n = n + 1
        # 触发判断定位点
        m = self.base.checkpoint_list(Excel_basedata_zs, sheet7)
        point = self.findxpath(self.base.checkpoint_dic(Excel_basedata_zs, sheet7).get(m[0]))
        # 旧环境的记录值
        p = len(m)
        if point.is_displayed():
            ...
            x = {}
            while i < p:
                o = self.findxpath(self.base.checkpoint_dic(Excel_basedata_zs, sheet7).get(m[i])).get_attribute('textContent')
                x.setdefault(m[i], o)
                i = i + 1
        else:
        ...
        y = {}
        while r < p:
            s = self.findxpath(self.base.checkpoint_dic(Excel_basedata_zs, sheet7).get(m[r])).get_attribute('textContent')
            y.setdefault(m[r], s)
            r = r + 1
        # 局部校验
        z = 0
