...
class NewProductContract(BasePageXams):
    ...
    def new_product_contract_compare(self, menu, value):
        ...
        while n < l:
            ...
            n = n + 1
        # 触发判断定位点
        m = self.base.checkpoint_list(sheet16)
        point = self.findxpath(self.base.checkpoint_dic(sheet16).get(m[0]))
        # 旧环境的记录值
        p = len(self.base.checkpoint_list(sheet16))
        if point.is_displayed():
            i = 0
            x = {}
            while i < p:
                o = self.findxpath(self.base.checkpoint_dic(sheet16).get(m[i])).get_attribute('textContent')
                x.setdefault(m[i], o)
                i = i + 1
        else:
        ...
        y = {}
        while r < p:
            s = self.findxpath(self.base.checkpoint_dic(sheet16).get(m[r])).get_attribute('textContent')
            y.setdefault(m[r], s)
            r = r + 1
        # 局部校验
        z = 0
