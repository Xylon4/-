...
class NewProductContract(BasePageXams):
    # 模拟操作自动化案例
    def new_product_contract_excel(self, menu, value):
        print(menu)
        print(value)
        self.base = TestExcel()
        # 点击一级菜单
    ...
    # 数据对比自动化案例
    def new_product_contract_compare(self, menu, value):
        print(menu)
        print(value)
        self.base = TestExcel()
        # 点击一级菜单
        ...
        while n < l:
            ...
            n = n + 1
        # 触发判断定位点
        m = self.base.checkpoint_list(Excel_basedata_zs, sheet16)
        point = self.findxpath(self.base.checkpoint_dic(Excel_basedata_zs, sheet16).get(m[0]))
        # 旧环境的记录值
        p = len(m)
        if point.is_displayed():
            ...
            x = {}
            while i < p:
                o = self.findxpath(self.base.checkpoint_dic(Excel_basedata_zs, sheet16).get(m[i])).get_attribute('textContent')
                x.setdefault(m[i], o)
                i = i + 1
        else:
        ...
        y = {}
        while r < p:
            s = self.findxpath(self.base.checkpoint_dic(Excel_basedata_zs, sheet16).get(m[r])).get_attribute('textContent')
            y.setdefault(m[r], s)
            r = r + 1
        # 局部校验
        z = 0
